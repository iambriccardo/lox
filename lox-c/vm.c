...
#define BINARY_OP(valueType, op)                                               \
  ...
  for (;;) {
    ...
    disassembleInstruction(
        ...
        (int)(frame_ip - frame->closure->function->chunk.code));
#endif
    ...
    case OP_CONSTANT_LONG: {
      ...
      int constant_index = ((int)bytes[2]) << 16;
      constant_index |= ((int)bytes[1]) << 8;
      constant_index |= bytes[0];
    ...
    case OP_SET_GLOBAL: {
      ObjString *name = READ_STRING();
      Value nameValue = OBJ_VAL(name);
      if (tableSet(&vm.globals, nameValue, peek(0))) {
        tableDelete(&vm.globals, nameValue);
        runtimeError("Undefined variable '%s'.", name->chars);
        return INTERPRET_RUNTIME_ERROR;
      }
      break;
